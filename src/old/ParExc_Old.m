...
n0 = 2; dn = 1; nf = 2; % dn não pode ser zero
...
espFaseU = true;  % plota espaço de fase de u x u' (junto com espectro)

espectroA = true; % false -> só os espaços de fase para os Aks 
espFaseA = true;  % plota espaço de fase de A x A' (junto com espectro)
...
g = [a(1) 
    ...
    b(6) 
    a(5) 
    diam^2*EA/(4*mu_d*(m+Ca)*w1^2)*(3*pi/L)^4];
...
for n=n0:dn:nf
    ...
    if ismember(n,n_plot)
        ...
        thistab = uitabgroup;%,'Position',[.05 .05 .3 .8]); % tabgroup
...
if respAmpli == true || respAk == true
    figure('units', 'normalized', 'position', pos, 'color', 'w');
    uitabgroup;%,'Position',[.05 .05 .3 .8]); % tabgroup
    hold on;
